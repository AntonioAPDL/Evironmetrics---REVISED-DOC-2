...
We write the full framework as one state-space model with time-dependent observation channels. Let
\[
\boldsymbol{\eta}_t =
\begin{bmatrix}
\boldsymbol{\theta}_t \\
...
Let \(\mathcal{A}_t\) denote the set of active observation channels at time \(t\). We take channel \(0\) to be the USGS series, channels \(1,\ldots,J\) to be the retrospective products during the estimation period, and channels \((j,i)\) to be the \(i\)th forecast member from source \(j\) after the cutoff. The full model is
\begin{align}
\mathcal{A}_t &=
\begin{cases}
\{0,1,\ldots,J\}, & 1 \le t \le T,\\
\{(j,i): 1 \le j \le J,\ 1 \le i \le I_j,\ t-T \le K_{(j)}\}, & T < t \le T+K_{(1)},
\end{cases}
\label{eq:activechannels}
\\
y_{t,s} \mid \boldsymbol{\eta}_t, \sigma_s, \gamma_s
&\sim \text{exAL}_{p_0}\!\left(\mathbf{u}_{t,s}' \boldsymbol{\eta}_t, \sigma_s, \gamma_s\right),
\qquad s \in \mathcal{A}_t,
\label{eq:fullobs}
\\
\mathbf{u}_{t,s} &=
\begin{cases}
\mathbf{h}_{t,0}, & s = 0,\\
\mathbf{h}_{t,j}, & s = j,\ 1 \le j \le J,\ 1 \le t \le T,\\
\mathbf{e}_{t,j}, & s = (j,i),\ T < t \le T+K_{(1)},
\end{cases}
\label{eq:fullselector}
\\
\boldsymbol{\eta}_t \mid \boldsymbol{\eta}_{t-1}
&\sim \mathcal{N}\!\left(\widetilde{\mathbf{D}}_t \boldsymbol{\eta}_{t-1}, \widetilde{\mathbf{W}}_t\right),
\label{eq:fullstate}
\\
(\widetilde{\mathbf{D}}_t,\widetilde{\mathbf{W}}_t) &=
\begin{cases}
(\boldsymbol{D}_t,\boldsymbol{\Lambda}_t), & 1 \le t \le T,\\
(\boldsymbol{M}_t,\mathbf{W}_t), & T < t \le T+K_{(1)}.
\end{cases}
\label{eq:fulltransition}
\end{align}
The observation-specific parameters are \((\sigma^o,\gamma^o)\) for the USGS series and \((\sigma^j,\gamma^j)\) for all channels associated with source \(j\), whether retrospective or forecast-based.

During the estimation period, the USGS observations and retrospective products are treated as source-specific views of the same latent quantile process. Their selectors are
\[
\mathbf{u}_{t,0} = \mathbf{h}_{t,0}, \qquad \mathbf{u}_{t,j} = \mathbf{h}_{t,j}, \quad 1 \le j \le J,
\]
with
\[
\mathbf{h}_{t,0} =
...
\quad 1 \le j \le J,
\]
so the USGS mean is the latent quantile \(\mathbf{F}_t' \boldsymbol{\theta}_t + \zeta_t\), whereas the \(j\)th retrospective source adds its discrepancy term \(\mathbf{F}_t' \boldsymbol{\delta}_t^j\). The estimation-period state evolution matrix and covariance are
\[
...
\]
After the cutoff, the same state vector is propagated forward and the active channels switch from retrospective products to forecast-product ensemble members. The selector \(\mathbf{e}_{t,j}\) is the forecast-window analogue of \(\mathbf{h}_{t,j}\): it extracts the latent trend-and-seasonal block, the discrepancy block for source \(j\), and, when active, the transfer block. The matrix \(\boldsymbol{M}_t\) propagates these same components through the forecast window. In the full T1 specification, the transfer states \((\zeta_t,\boldsymbol{\psi}_t)\) remain active and the forecast covariates available at the cutoff are propagated through the transition block. The reduced T0 specification is obtained by suppressing that forecast-window transfer block after the cutoff.

This single model is algebraically equivalent to the earlier Models A, B, and C. Model A is obtained by suppressing the discrepancy states and retaining only channel \(0\). Model B is obtained by keeping channels \(0,1,\ldots,J\) and restricting attention to \(1 \le t \le T\). Model C is the full specification in \eqref{eq:activechannels}--\eqref{eq:fulltransition}, where the active channel set and transition pair switch after the cutoff. In implementation, we work with reduced forecast-window matrices that retain only the forecasters whose horizons remain active at each lead time, but that reduction is only computational and does not change the model itself.
