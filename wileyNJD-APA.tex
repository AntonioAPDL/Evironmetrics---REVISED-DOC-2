...
\begin{align}
     \text{USGS observations}:&& y^o_t \mid \boldsymbol{\mu}_t, \sigma^o, \gamma^o &\sim \text{exAL}_{p_0}\!\left(\mathbf{h}_{t,0}' \boldsymbol{\mu}_t, \sigma^o, \gamma^o\right), \label{B5} \\
    \text{Retrospective products}:&& z^j_t \mid \boldsymbol{\mu}_t, \sigma^j, \gamma^j &\sim \text{exAL}_{p_0}\!\left(\mathbf{h}_{t,j}' \boldsymbol{\mu}_t, \sigma^j, \gamma^j\right), \quad 1 \leq j \leq J, \label{B6} \\
    \text{All states}:&& \boldsymbol{\mu}_t \mid \boldsymbol{\mu}_{t-1} &\sim \mathcal{N}\!\left(\boldsymbol{D}_t \boldsymbol{\mu}_{t-1}, \boldsymbol{\Lambda}_t\right). \label{B7}
\end{align}
Here,
\[
\mathbf{h}_{t,0} =
...
\begin{align}
    \text{Forecast products}:&& y_T^{j,i}(k) \mid \boldsymbol{\mu}_{T+k}, \sigma^j, \gamma^j &\sim \text{exAL}_{p_0}\!\left(\mathbf{e}_{T+k,j}' \boldsymbol{\mu}_{T+k}, \sigma^j, \gamma^j\right), \label{C1} \\
    \text{Forecast-window states}:&& \boldsymbol{\mu}_{T+k} \mid \boldsymbol{\mu}_{T+k-1} &\sim \mathcal{N}\!\left(\boldsymbol{M}_{T+k} \boldsymbol{\mu}_{T+k-1}, \mathbf{W}_{T+k}\right), \label{C2} \\
...
For presentation, \eqref{B5}--\eqref{C2} define one model with time-dependent active observation channels. In implementation, we use reduced forecast-window vectors and matrices that retain only the forecasters with horizons still active at a given lead time. This reduced representation is computationally convenient, but it is algebraically equivalent to the single full framework presented here.
...
\(
\mathbb{E}[\mathbf{W}_t \mid \mathcal{D}_t] = \frac{\epsilon}{1 + \epsilon}(c\, \mathbf{W}_T) + \frac{1}{1 + \epsilon} \mathbb{E}\left[ (\boldsymbol{\mu}_t - \mathbf{M}_t \boldsymbol{\mu}_{t-1}) (\boldsymbol{\mu}_t - \mathbf{M}_t \boldsymbol{\mu}_{t-1})' \mid \mathcal{D}_t \right],
\)
