...
The source-specific parameters are \((\sigma^o,\gamma^o)\) for the USGS series and \((\sigma^j,\gamma^j)\) for all channels associated with source \(j\), whether retrospective or forecast-based. Here \(J\) denotes the number of external product families, \(I_j\) is the number of forecast members available from source \(j\), and \(K_{(j)}\) is the forecast horizon for that source. We write \(K_{\max} = \max_{1 \le j \le J} K_{(j)}\) for the longest forecast horizon across sources. The symbol \(T\) denotes the cutoff or forecast origin, so \(y_T^{j,i}(k)\) is the \(k\)-step-ahead member issued at that origin by member \(i\) of source \(j\).

The first two lines correspond to the estimation period, where the available observations are the USGS series and the retrospective products. The third line corresponds to the forecast window, where those channels are replaced by the forecast products issued at or before the cutoff.
...
For later reference, the same model admits a compact matrix form. In the estimation period, the active channels are the USGS series and the retrospective products:
\begin{align}
...
\]
Here \(\mathbf{G}_t\) is repeated once for the latent trend-and-seasonal block and once for each discrepancy block. Thus, \(\mathbf{h}_{t,0}' \boldsymbol{\eta}_t = \mathbf{F}_t' \boldsymbol{\theta}_t + \zeta_t\) and \(\mathbf{h}_{t,j}' \boldsymbol{\eta}_t = \mathbf{F}_t'(\boldsymbol{\theta}_t + \boldsymbol{\delta}_t^j) + \zeta_t\).

In the forecast window, the active channels are the forecast products:
\begin{align}
...
\end{align}
Here \(\mathbf{e}_{t,j}\) is the forecast-window analogue of \(\mathbf{h}_{t,j}\): it extracts the latent trend-and-seasonal block, the discrepancy block for source \(j\), and, in the T1 specification, the transfer block. The matrix \(\boldsymbol{M}_t\) preserves the same block ordering as \(\boldsymbol{D}_t\) while propagating the state through the forecast window. In computation, we use reduced forecast-window matrices that retain only the forecasters whose horizons remain active at each lead time; this reduction is computational only and does not change the underlying model.

This single formulation also contains familiar special cases. Suppressing the discrepancy states and retaining only the USGS observation channel yields an observation-only specification. Keeping the USGS and retrospective channels but restricting attention to \(1 \le t \le T\) yields an estimation-period multivariate specification. The full model in \eqref{eq:unified_usgs}--\eqref{eq:compact_fc_state} continues the same latent state into the forecast window while changing the active observation channels after the cutoff.
