...
The compact form is included to highlight the computational structure of the model. Under the stochastic representation in Subsection~\ref{subsec:exal}, the observation equations are conditionally Gaussian given the latent augmentation variables, while the state evolution remains Gaussian. Standard state-space tools, including filtering, smoothing, and FFBS recursions, can therefore be applied directly in posterior computation. The compact display is intended to make that structure transparent, rather than to introduce a second formulation of the model.
