...
We study this framework using daily river flow for the San Lorenzo River together with retrospective and forecast products from the European Centre for Medium-Range Weather Forecasts (ECMWF) Global Flood Awareness System (GloFAS) and the National Oceanic and Atmospheric Administration (NOAA) National Weather Service (NWS). The empirical focus is forecasting performance and uncertainty quantification across multiple quantile levels, rather than only historical fit or methodological development in isolation. Although the application is hydrological, the same structure is relevant in other settings where observations, retrospective products, and forecast products all play distinct roles.
...
We first write the full T1 specification in expanded form, where T1 denotes the version in which the transfer component remains active after the cutoff:
\begin{align}
...
Here \(\mathbf{F}_t \in \mathbb{R}^{p}\), \(\mathbf{G}_t \in \mathbb{R}^{p\times p}\), \(\mathbf{W}_t^\theta \in \mathbb{R}^{p\times p}\), \(\mathbf{W}_t^{\delta^j} \in \mathbb{R}^{p\times p}\), and \(\mathbf{W}_t^\psi \in \mathbb{R}^{m\times m}\). The source-specific parameters are \((\sigma^o,\gamma^o)\) for the observed river-flow series and \((\sigma^j,\gamma^j)\) for all channels associated with source \(j\), whether retrospective or forecast-based. Here \(J\) denotes the number of external product families, \(I_j\) is the number of forecast members available from source \(j\), and \(K_{(j)}\) is the forecast horizon for that source. We write \(K_{\max} = \max_{1 \le j \le J} K_{(j)}\) for the longest forecast horizon across sources. The symbol \(T\) denotes the cutoff or forecast origin, so \(y_T^{j,i}(k)\) is the \(k\)-step-ahead member issued at that origin by member \(i\) of source \(j\).
...
The last three lines define the block structure of the state evolution. In the compact form below, these blocks are assembled into \(\boldsymbol{D}_t\) and \(\boldsymbol{\Lambda}_t\) before the cutoff and into \(\boldsymbol{M}_t\) and \(\mathbf{W}_t\) after the cutoff. In the T1 specification, the transfer block remains active after the cutoff, with \(\mathbf{x}_t\) given by the forecast covariates available at that origin. The benchmark label T0, introduced in Section~\ref{subsec:benchmarkresults}, denotes the corresponding restriction in which the forecast-window transfer block is suppressed after the cutoff.

For later reference, the same model admits the following compact matrix form:
\begin{align}
...
\quad 1 \le j \le J,\ 1 \le i \le I_j,\ T < t \le T + K_{(j)}, \label{eq:compact_fc_obs}
\\
\boldsymbol{\eta}_t \mid \boldsymbol{\eta}_{t-1}
&\sim \mathcal{N}\!\left(\boldsymbol{M}_t \boldsymbol{\eta}_{t-1}, \mathbf{W}_t\right),
\qquad T < t \le T + K_{\max}. \label{eq:compact_fc_state}
\end{align}
The first two lines describe the estimation period, whereas the last two lines describe the forecast window. Thus, \eqref{eq:compact_est_obs}--\eqref{eq:compact_fc_state} restate the expanded model above in one unified notation.
Here
\[
\mathbf{h}_{t,0} =
\begin{bmatrix}
...
\quad 1 \le j \le J,
\]
with \(\mathbf{h}_{t,0}, \mathbf{h}_{t,j} \in \mathbb{R}^{q}\), and
\[
...
\]
Here \(\boldsymbol{D}_t, \boldsymbol{\Lambda}_t \in \mathbb{R}^{q\times q}\), and \(\mathbf{G}_t\) is repeated once for the latent trend-and-seasonal block and once for each discrepancy block. Thus, \(\mathbf{h}_{t,0}' \boldsymbol{\eta}_t = \mathbf{F}_t' \boldsymbol{\theta}_t + \zeta_t\) and \(\mathbf{h}_{t,j}' \boldsymbol{\eta}_t = \mathbf{F}_t'(\boldsymbol{\theta}_t + \boldsymbol{\delta}_t^j) + \zeta_t\).

For the forecast window, \(\mathbf{e}_{t,j} \in \mathbb{R}^{q}\) is the analogue of \(\mathbf{h}_{t,j}\): it extracts the latent trend-and-seasonal block, the discrepancy block for source \(j\), and, in the T1 specification, the transfer block. The matrices \(\boldsymbol{M}_t, \mathbf{W}_t \in \mathbb{R}^{q\times q}\) preserve the same block ordering as \(\boldsymbol{D}_t\) while propagating the state through the forecast window. In computation, we use reduced forecast-window matrices that retain only the forecasters whose horizons remain active at each lead time; this reduction is computational only and does not change the underlying model.

This single formulation also contains familiar special cases. Suppressing the discrepancy states and retaining only the observation channel yields an observation-only specification. Keeping the observation and retrospective channels but restricting attention to \(1 \le t \le T\) yields an estimation-period multivariate specification. The full model in \eqref{eq:unified_usgs}--\eqref{eq:compact_fc_state} continues the same latent state into the forecast window while changing the active observation channels after the cutoff.
...
\caption{
Daily water flow (log-log scale, cm$^3$/s) of the San Lorenzo River at the Big Trees USGS station from May 29, 1987, to December 25, 2022. Observed daily flow (green) is displayed on the $\log(\log(x+1))$ scale. Vertical dashed lines and numbered labels indicate major flood-related events and infrastructure changes relevant to the study period: (1) February 1998 flood, (2) levee and floodwall reconstruction (2004), (3) February 2017 flood, and (4) the January 2023 flood immediately after the end of the recorded series. Horizontal dashed lines represent NOAA-defined minor (16.5 ft, 503 cm) and major (21.76 ft, 663 cm) flooding thresholds, both converted to the log-log scale.
}
...
\caption{
Scaled daily series of local precipitation, local soil moisture estimates, and the first generalized dynamic principal component derived from multiple climate and environmental indices. All series have been standardized for comparability.
}
...
\caption{Retrospective products from NWS and GloFAS at the San Lorenzo Big Trees USGS station.}
...
\caption{
\textbf{Before 2022-12-26:} Green—USGS observations; Purple—NWS retrospective product; Orange—GloFAS retrospective product. \textbf{From 2022-12-26 onward:} Red—future USGS observations used only for verification; Purple—NWS 7-member forecast product, covering 2022-12-25 to 2023-01-05; Orange—GloFAS 51-member forecast product, covering 2022-12-25 to 2023-01-22. Horizontal dashed lines indicate official flood-stage thresholds for the river.
}
\label{fig:ensembles}
\end{figure*}
...
The first GDPC factor remains negative and comparatively small after conditioning on precipitation and soil moisture, so its role is secondary to the local hydrometeorological covariates. These estimates indicate that the transfer component is driven primarily by precipitation and antecedent wetness, with the climate factor acting as a broader large-scale adjustment.
...
Figures~\ref{fig:dry_quantile} and \ref{fig:rainy_quantile} show clear differences between the drought and wet periods. During 2012--2016, the posterior bands remain comparatively narrow and the upper tail rarely approaches the flood thresholds. During 2017--2019, the upper tail widens substantially during the main winter events, and the 95th quantile exceeds the major flood stage in 2017 and 2019.
...
Figure~\ref{fig:synth1} shows the synthesized predictive distribution for the December 25, 2022 cutoff. It illustrates how the fitted quantile-specific forecasts combine into one predictive distribution, whereas the comparative forecast evaluation remains the main empirical evidence of the paper.
...
\end{tabular*}
\begin{tablenotes}
\item \textit{Note:} Entries show the posterior median and the 95\% credible interval $[{\rm Q2.5}, {\rm Q97.5}]$ for each source and quantile. USGS = U.S. Geological Survey; GloFAS = Global Flood Awareness System; NWS = National Weather Service.
\end{tablenotes}
\end{table*}
...
\)
\State For compactness, \(y_t^j\) denotes the active observation or product associated with source \(j\), and the corresponding selector vector is written generically as \(\mathbf{h}_{t,j}\); in the forecast window this selector is given by \(\mathbf{e}_{t,j}\) for each active source.
...
\State \textbf{Notation:}
\State  $\mathbf{H}_t = [\mathbf{h}_{t,0}, \mathbf{h}_{t,1}, \ldots, \mathbf{h}_{t,J}]$, $\mathbf{Y}_t = [y_t^o, z_t^1, \ldots, z_t^J]'$, and $\mathbf{D}_t, \boldsymbol{\Lambda}_t$ are as defined in Subsection~\ref{subsec:fullmodel}
\State  For the source-indexed updates below, let \(y_t^0 \equiv y_t^o\) and \(y_t^j \equiv z_t^j\) for \(1 \le j \le J\).
